...
import { useGetDocumentListQuery, useGetUserPermissionsQuery } from '@/services/apiSlice';
...
import { WorkflowStatus, DocumentListResponse } from '@/interfaces/DocumentList';
...
const DocumentsView: React.FC = () => {
  ...
  const [selectedDocForComments, setSelectedDocForComments] = useState<DocumentListResponse | null>(null);
  const [wallCrossSheetOpen, setWallCrossSheetOpen] = useState(false);
  const [selectedDocForWallCross, setSelectedDocForWallCross] = useState<DocumentListResponse | null>(null);
  ...
  const { data: allDocsResponse } = useGetDocumentListQuery(allDocumentsParams!, {
  ...
  const permissionsParams = accountName && srvrID && userID && accountID
    ? { AccountName: accountName, SrvrID: srvrID, UserID: userID, AccountID: accountID }
    : null;

  const { data: permissionsResponse } = useGetUserPermissionsQuery(permissionsParams!, {
  ...
  const statusCounts: Record<WorkflowStatus, number> = useMemo(() => {
    ...
    return {
      ...
      Final: allDocs.filter((d) => d.StatusName === 'Final' || d.StatusName === 'Finalised').length,
      Finalised: 0,
  ...
  const handleSelectionChange = useCallback((docId: number, selected: boolean) => {
  ...
  const handleOpenCommentsSheet = useCallback((document: DocumentListResponse) => {
  ...
  const handleOpenWallCrossSheet = useCallback((document: DocumentListResponse) => {
